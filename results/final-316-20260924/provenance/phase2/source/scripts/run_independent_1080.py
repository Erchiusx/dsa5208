...
PREVIOUS = OUT.parent / 'independent-1080-20260923'
CASES = []
...
def run():
    ...
    manifest = json.loads((OUT / 'resume-manifest.json').read_text())
    assert hashlib.sha256((PREVIOUS / 'plan.json').read_bytes()).hexdigest() == manifest['original_plan_sha256']
    for path, digest in manifest['copied_file_hashes'].items():
        assert hashlib.sha256((OUT / path).read_bytes()).hexdigest() == digest, path
    summaries = json.loads((PREVIOUS / 'summary.json').read_text())['cases'][:manifest['copied_completed_cases']]
    records = []
    for planned, meta in zip(CASES, summaries):
        assert meta['case_id'] == planned[0] and meta['evidence_verified'] and meta['recovery_verified']
        folder = OUT / meta['case_id']
        ...
        verify_record(record, controls, {n: v['ip'] for n, v in environment['nodes'].items()})
    ...
    save(OUT / 'plan.json', {
        ...
        'resumed_from': PREVIOUS.name, 'carried_forward_cases': len(summaries),
        'preparation_policy': manifest['protocol_change'],
    ...
    started = time.perf_counter() - manifest['prior_elapsed_seconds']
